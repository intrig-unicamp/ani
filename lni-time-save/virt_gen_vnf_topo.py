from virt_builder import InfraBuilder, Converter, ConverterV2, ConverterV3
...
class GeneratorV3:
    def __init__(self):
        self.converter = ConverterV3('BisBis', 'BisBis-View')

    def get_virt(self, nodes_final, edges_final):
        virt = self.converter.get(nodes_final, edges_final)
        return virt

    def get(self, graph):
        self.converter.fill_nodes(graph)
        self.converter.fill_edges(graph)
        return graph
...
def generate_ani_node_oriented(intModel, nfs, delay_req, bw_req, graph, iter_nf):
  ...
  virt.write_to_file(strFolderADD+'/ani-model-'+str(intModel)+'-node.xml')
...
def generate_ani_edge_oriented(intModel, nfs, delay_req, bw_req, graph, iter_nf):
  ...
  str_num_ns=str(num_ns)
  strFolderADD=strFolder+'ns_req_files_add/'+'vnfs'+str_num_ns
  virt.write_to_file(strFolderADD+'/ani-model-'+str(intModel)+'-edge.xml')
...
if __name__ == "__main__":
    graph_aux=nx.read_gml('/local/dlachosper/ownCloud/thesis/ani/evaluation/ani/eval-time/real-topo/Interoute.gml', label='id')
    #graph_aux=nx.read_gml('/home/dlachosper/ownCloud/thesis/ani/evaluation/ani/eval-time/real-topo/Interoute.gml', label='id')
    virt_gen = GeneratorV3()
    graph = virt_gen.get(graph_aux)
    virt = virt_gen.get_virt(graph.nodes_iter(data=True), graph.edges_iter(data=True))

    model = 'real'

    for numNF in range (2,11):
      #strSed = 'sed -i "s/NRO_VNFS_X_REQ.*/NRO_VNFS_X_REQ = '+str(numNF)+'/g" defs.py'
      #os.system(strSed)
      NRO_VNFS_X_REQ = numNF
      ...
      virt.write_to_file(strFolderADD+'/model-real-topo.xml')
      nx.write_gml(graph, strFolderADD+'/full-graph-model-real-topo.gml')

      os.system('mkdir '+strFolderADD+'/'+str(numNF))
      shutil.copytree(strFolderADD+'/ns_req_files_add/', strFolderADD+'/'+str(numNF)+'/'+'ns_req_files_add')
      shutil.copytree(strFolderADD+'/ns_req_files_del/', strFolderADD+'/'+str(numNF)+'/'+'ns_req_files_del')
      shutil.copy(strFolderADD+'/full-graph-model-real-topo.gml', strFolderADD+'/'+str(numNF)+'/full-graph-model-real-topo.gml')
      shutil.copy(strFolderADD+'/model-real-topo.xml', strFolderADD+'/'+str(numNF)+'/model-real-topo.xml')



    #GeneratorV3.fill_nodes(graph)
    #GeneratorV3.fill_edges(graph)
    #print nx.info(graph)

    #virt = virt_gen.get_virt(self.graph.nodes_iter(data=True), self.graph.edges_iter(data=True))
    '''

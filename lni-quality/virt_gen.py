...
def generate_ani_node_oriented(intModel, nfs, delay_req, bw_req, graph):
  #threshold = 0.75
  edges_final = []
  nodes_final = []
...
def generate_ani_edge_oriented(intModel, nfs, delay_req, bw_req, graph):
  ###########Compute minimo
  #for i in range(len(nfs)-1):
  #bw_req = MIN ()
...
def generate_ani(intModel, ns_name, nfs, delay_req, bw_req, graph):
...
def generate_ns_req (intModel, graph, ns_name):
  ...
  delay_req=140
  bw_req=random.randint(FLOW_BANDWIDTH_MIN, FLOW_BANDWIDTH_MAX)#4

  nodes_final_e, edges_final_e, ani_graph_e = generate_ani_edge_oriented(intModel, v_nf, delay_req, bw_req, graph)
  nodes_final_n, edges_final_n, ani_graph_n = generate_ani_node_oriented(intModel, v_nf, delay_req, bw_req, graph)
  nodes_final, edges_final, ani_graph = generate_ani(intModel, ns_name, v_nf, delay_req, bw_req, graph)
  ...
  outputFileNamee =strFoldere+'out-model'+str(intModel)+'.txt'
  ...
  outputFileNamen =strFoldern+'out-model'+str(intModel)+'.txt'
  ...
  outputFileName =strFolder+'out-model'+str(intModel)+'.txt'

  os.system('echo "%d %d %s %s %d %d %s %s" >> %s' % (val1,val2,str(val3).replace('.',','),str(valDen1).replace('.',','), val4e,val5e,str(val6e).replace('.',','),str(valDen2e).replace('.',','),outputFileNamee))
  os.system('echo "%d %d %s %s %d %d %s %s" >> %s' % (val1,val2,str(val3).replace('.',','),str(valDen1).replace('.',','), val4n,val5n,str(val6n).replace('.',','),str(valDen2n).replace('.',','),outputFileNamen))
  os.system('echo "%d %d %s %s %d %d %s %s" >> %s' % (val1,val2,str(val3).replace('.',','),str(valDen1).replace('.',','), val4,val5,str(val6).replace('.',','),str(valDen2).replace('.',','),outputFileName))
